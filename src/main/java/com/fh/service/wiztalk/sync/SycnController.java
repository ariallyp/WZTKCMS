 ...
 * All rights reserved.
 */
package com.fh.service.wiztalk.sync;
 ...
 */
@Controller
public class SycnController extends BaseController {
	...
	// @ResponseBody
	public String syncd() throws Exception {
		long beginMillis = System.currentTimeMillis();
		logger.info("开始调用数据库同步syncDbService.syncdb()方法时的毫秒数为：" + beginMillis);

		syncDbService.syncdb();

		long endMillis = System.currentTimeMillis();
		logger.info("数据库同步syncDbService.syncdb()方法调用结束的毫秒数为：" + endMillis);
		logger.info("调用数据库同步syncDbService.syncdb()方法一共花费了：" + (endMillis - beginMillis) + "毫秒");
	...
	private void timedb(String str) throws Exception {
		...
		try {
			...
			JobDetail jobDetail = JobBuilder.newJob(SynUser.class).withDescription("Easc sync user job") // job的描述
					.withIdentity("synUser", "ramGroup") // job
					.build();
			long time = System.currentTimeMillis() + 3 * 1000L; // 3秒后启动任务
			Date statTime = new Date(time);
			Trigger trigger = TriggerBuilder.newTrigger().withDescription("")
					.withIdentity("ramTrigger", "ramTrigger").startAt(statTime) // 默认当前时间启动
					.build();
