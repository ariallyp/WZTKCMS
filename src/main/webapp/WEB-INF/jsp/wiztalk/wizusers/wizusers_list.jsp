...
<body>
		...
		<div id="page-content" class="clearfix">
			...
			<div class="row-fluid">
				...
				<div class="row-fluid">
					...
					<!-- 检索  -->
					<form action="wizusers/list.do?ORG_ID=${ORG_ID }" method="post"
						name="Form" id="Form">
						...
						<table>
							<tr>
								...
								<td style="vertical-align: top;"><button
										class="btn btn-mini btn-light" id="resetBtn" title="重置">重置</button></td>
								<%-- <c:if test="${QX.cha == 1 }">
											...
											<c:forEach items="${varList}" var="var" varStatus="vs">
												<tr>
													<td class='center' style="width: 30px;"><label><input
															type='checkbox' name='ids' value="${var.WIZUSERS_ID}" /><span
							...
							<table style="width: 100%;">
								<tr>
									<td style="vertical-align: top;"><c:if
											test="${!empty ORG_ID }">
											<a id="addUser" class="btn btn-small btn-success"
												onclick="add();">新增</a>
										</c:if> <c:if test="${QX.add == 1 }">
											<a class="btn btn-small btn-success"
												onclick="addAPP('确定要添加应用吗?');">添加应用</a>
	...
	<script type="text/javascript">
		...
		function edit(Id){
			 ...
			 diag.Title ="编辑";
			 //ORG_ID='+"${ORG_ID }"
			 diag.URL = '<%=basePath%>wizusers/goEdit.do?WIZUSERS_ID='+Id+'&ORG_ID='+"${ORG_ID }";
